...
export async function fetchStockQuote(symbol) {
    ...
    try {
        ...
        const error = new Error('Could not connect to the stock data service. Please check your internet connection.');
    ...
    if (res.status === 402) {
        const error = new Error('This endpoint requires a paid API plan');
        error.type = 'INVALID_API_KEY';
        throw error;
    }
    ...
    if (!res.ok) {
        const error = new Error(`Unexpected error from the API (status ${res.status}). Please try again later.`);
    ...
    if (!result) {
        const error = new Error(`No stock data found for symbol "${s}". Please check the ticker and try again.`);
